  ...
  <div class='passingCheck'>
    <div class='a_lists'  v-for='(item,index) in lists'>
      <h1 class='q_title textBig' @click='isShow(index)' v-show='show_a & index === activeIndex'>
        <div class='gainGold' v-show='show_a'>获得：<em class='goldBig'></em><a>{{item.integralGain}}</a></div>{{item.question}}<span
      ...
      <el-collapse-transition>
        <div v-show='show_a & index === activeIndex'>
          ...
          <div class='myAnswer'>
            <h2 class='a_top'><i class='lamp'></i>我的回答<a class='corrected'>平台纠错</a><span class='a_times'>{{getLocalTime(item.createTime)}}</span>
            </h2>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDetail () {
        var _this = this
        this.$indexServer.auditSuccess(_this.pageNum,_this.pageSize)
          .then((res)=>{
